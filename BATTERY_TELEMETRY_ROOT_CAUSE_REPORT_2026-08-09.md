...
> **後續修正狀態（2026-08-17）：** `nrf52_promicro_diy_xtal` 已改用單一 UF2 自動支援兩種來源：優先使用 `1 MΩ / 1 MΩ`、分壓比 `0.5`、ADC 反算倍率 `2.0` 的 P0.31/AIN7 外部分壓，未偵測到分壓器時才在 VDDH 高電壓模式使用 nRF52840 內建 `VDDH/5`；SAADC 取樣時間為 40 µs。VDDH-only 在 USB 插入時會誠實回報電池未知。下文的 `1.0 MΩ / 1.5 MΩ`、`0.6`、`1.73` 與衍生數值是固定版本 `9306d3f55`／`7c6b85d` 的歷史診斷證據，不代表後續建置的板型設定。
