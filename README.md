...
`nrf52_promicro_diy_xtal` 的同一份 UF2 支援兩種單節鋰電池量測方案，不需要修改 App 或選擇設定。韌體會在開機後第一次量測時自動判定來源，並固定使用該判定直到下次重開機。
...
```text
VBAT / B+ ── 1 MΩ ──┬── 1 MΩ ── GND / B−
                     │
                     └── P0.31 / AIN7
```

- 兩顆電阻建議使用相同精度，例如 `1%`。
- P0.31 與 GND 間建議加一顆約 `100 nF` 的陶瓷電容，以穩定高阻分壓的 ADC 讀值。
- 分壓比為 `0.5`，韌體預設 ADC 反算倍率為 `2.0`，SAADC 取樣時間為 `40 µs`。
- 不可將最高可達 4.2 V 的電池正極直接接到 P0.31，也不可將分壓器接到 USB 5 V。
- 若裝置曾設定 `power.adc_multiplier_override`，請將它清為 `0` 以使用板型預設值，或直接設為 `2.0`；這個設定只作用於外部分壓方案。

### 方案二：nRF52840 VDDH/5

此方案不使用 P0.31，也不需要外部感測電阻。PCB 必須讓電池供電實際進入 nRF52840 的 `VDDH`，韌體才能透過晶片內建 `VDDH/5` 量到電池；若板上沒有這條電源連線，僅更新韌體不會產生電池讀值。早期 nRF52840 Engineering A 的 VDDHDIV5 硬體異常不支援此方案。

### 自動選擇與 USB 行為

- 韌體先以 P0.31 的安全拉低探測檢查外部分壓；兩種方案同時存在時，永遠優先使用外部分壓。
- 未偵測到外部分壓時，只有晶片確認目前由 VDDH 高電壓模式供電，才使用 `VDDH/5`；其餘情況回報電池未知。
- 若外部分壓方案暫時讀值無效、未插 USB 且 VDDH 可用，該次讀值可安全改由 `VDDH/5` 取得，但不改變開機時鎖定的主要來源。
- SuperMini 的原始電源路徑在插入 USB 後會讓 VDDH 接近 USB 電壓，因此 `VDDH/5` 方案此時會顯示 `PWR`，且不附上假的電池電壓；拔除 USB 後會在下一個量測／遙測週期恢復。
- 外部分壓方案可在插入 USB 時繼續量到真正的電池電壓；App 仍依既有規則顯示 `PWR`，遙測封包則保留電池電壓。
- 更換分壓器或 VDDH 供電方式後必須重開機，讓韌體重新探測。

### 萬用電表驗證

外部分壓方案在電池為 `3.30 / 3.70 / 4.20 V` 時，P0.31 對 GND 應約為 `1.65 / 1.85 / 2.10 V`。VDDH 方案則應先確認電池供電時 VDDH 與電池正端電壓相近；再分別於 USB 拔除與插入時比對 App／遙測，避免把 USB 電壓誤認為電池。量測有效範圍為 `2.5–4.5 V`，初始硬體驗收目標為韌體讀值與萬用電表相差不超過 `±0.15 V`。

| 硬體方案 | 只用電池供電 | 插入 USB |
| --- | --- | --- |
| P0.31 雙 1 MΩ | 顯示電壓與 0–100% | 顯示 PWR，遙測仍附真實電池電壓 |
| VDDH/5 | 顯示電壓與 0–100% | 顯示 PWR，不附電池電壓 |
