...
static uint8_t getEffectiveRegionPowerLimit(const RegionInfo *region)
{
#ifdef LORA_TW_POWER_LIMIT_OVERRIDE
    if (region && region->code == meshtastic_Config_LoRaConfig_RegionCode_TW)
...
void RadioInterface::applyModemConfig()
{
    ...
    const uint8_t regionPowerLimit = getEffectiveRegionPowerLimit(newRegion);
...
void RadioInterface::limitPower(int8_t loraMaxPower)
{
    ...
    const uint8_t regionPowerLimit = getEffectiveRegionPowerLimit(myRegion);
    if (regionPowerLimit)
